...
function RayfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = "Loading cracked library..."
	LoadingFrame.Subtitle.Text = "by xsinew"
	...
	if Settings.KeySystem then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs({"cracked by xsinew"}) do
				local Success, Response = pcall(function()
					Settings.KeySettings.Key[i] = tostring(game:HttpGet(Key):gsub("[\n\r]", " "))
					Settings.KeySettings.Key[i] = string.gsub(Settings.KeySettings.Key[i], " ", "")
				end)
				if not Success then
					print("CrackedXsi | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs({"cracked by xsinew"}) do
		...
		if not Passthrough then
			...
			KeyMain.Title.Text = "Cracked Key System"
			KeyMain.Subtitle.Text = "크시가 크랙했다냥"
			KeyMain.NoteMessage.Text = "키: cracked by xsinew"
