local ui_options = {
	main_color = Color3.fromRGB(155, 113, 254),
	min_size = Vector2.new(300, 230),
	toggle_key = Enum.KeyCode.RightShift,
	can_resize = false,
}

do
	local imgui = game:GetService("CoreGui"):FindFirstChild("크시툴")
...
function SetText(text)
    for idx, plr in pairs(ps:GetPlayers()) do
        if table.find(excludeList, plr) then continue end
        local char = plr.Character
        if char then
            local Remote = char:FindFirstChild("UpdateSign", true)
            if Remote then
                Remote:FireServer(string.gsub(text, "${player}", plr.Name))
            end
        end
    end
end

local Window = library:AddWindow("검열없는호텔 어드민 패널")
local MainTab = Window:AddTab("표지판")
local ExcludeTab = Window:AddTab("표지판제외")
local AuraTab = Window:AddTab("아우라착용")

local textInput = MainTab:AddConsole({
	["y"] = 110,
	["source"] = "Lua",
	["readonly"] = false,
	["full"] = false,
...
local hBar = MainTab:AddHorizontalAlignment()

hBar:AddButton("켜기", function()
	isEnabled = true
	task.spawn(function()
		while isEnabled and task.wait(0.15) do
			SetText(textInput:Get())
		end
	end)
end)

hBar:AddButton("끄기", function()
	isEnabled = false
end)
...
for idx, plr in pairs(ps:GetPlayers()) do
	print(plr.Name)
	local sw = ExcludeTab:AddSwitch(plr.DisplayName, function(bool)
		if bool then
			table.insert(excludeList, plr)
		else
			local fIndex = table.find(excludeList, plr)
			if fIndex then
				table.remove(excludeList, fIndex)
			end
		end
	end)
	table.insert(plrSwitch, sw)
end

ps.PlayerAdded:Connect(function (plr)
	table.insert(plrSwitch, ExcludeTab:AddSwitch(plr.DisplayName, function(bool)
		if bool then
			table.insert(excludeList, plr)
		else
			local fIndex = table.find(excludeList, plr)
...
ps.PlayerRemoving:Connect(function (plr)
	...
	if fIndex then
		table.remove(excludeList, fIndex)
		table.remove(plrSwitch, table.find(plrSwitch, plr))
	end
end)

function EquipAura(name)
	game:GetService("ReplicatedStorage").AR:FireServer("Unequip")
	game:GetService("ReplicatedStorage").AR:FireServer("Equip", name)
end

AuraTab:AddButton("Unequip", function ()
	game:GetService("ReplicatedStorage").AR:FireServer("Unequip")
end)
for idx, aura in pairs(game:GetService("ReplicatedStorage").Auras:GetChildren()) do
	AuraTab:AddButton(aura.Name, function ()
		EquipAura(aura.Name)
	end)
end
